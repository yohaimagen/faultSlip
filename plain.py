...
class Plain:
        ...
        if not sources_file:
            for i in range(len(width)):
                ...
                for j in range(int(num_sub_stk[i])):
                    self.sources.append(Source(None, None, np.deg2rad(strike), np.deg2rad(dip), source_length,
                                                 width[i], 0, 0, 0, source_length * j + source_length / 2.0, self.total_width,
                                                 plain_cord[0], plain_cord[1], plain_cord[2]))
        else:
            sr_mat = np.load(sources_file)
            for i in range(sr_mat.shape[0]):
                self.sources.append(Source(None, None, np.deg2rad(sr_mat[i, 4]), np.deg2rad(sr_mat[i, 3]),
                                             sr_mat[i, 0], sr_mat[i, 1], 0, 0, 0, sr_mat[i, 12], sr_mat[i, 13],
                                             east=sr_mat[i, 10], north=sr_mat[i, 11], depth=sr_mat[i, 2]))
    ...
    def get_strike_ker(self, zero_pad=0):
        ...
        if zero_pad != 0:
            strike_kernal_temp = np.zeros((self.strike_kernal.shape[0], self.strike_kernal.shape[1] + zero_pad))
    ...
    def get_dip_ker(self, zero_pad=0):
        ...
        if zero_pad != 0:
            dip_kernal_temp = np.zeros((self.dip_kernal.shape[0], self.dip_kernal.shape[1] + zero_pad))
    ...
    def get_smothing_no_bounds(self):
        ...
        left_x = np.array([s_plain.x - 0.5 * s_plain.length for s_plain in self.sources])
        right_x = np.array([s_plain.x + 0.5 * s_plain.length for s_plain in self.sources])
    ...
    def plot_sources(self, movment, ax, my_cmap=None, norm=None, I=False):
        for i, sr in enumerate(self.sources):
            ...
            ccw_to_x_stk = np.pi / 2 - sr.strike  # the angle betuen the fualt and the x axis cunter clock wise
            ...
            ax.plot_surface(np.array([[e1, e2], [e3, e4]]), np.array([[n1, n2], [n3, n4]]),
                            np.array([[z1, z1], [z2, z2]]), color=color_sub_plain, edgecolor='black')
    ...
    def plot_sources_2d(self, ax, movment, my_cmap=None, norm=None):
        for i, sr in enumerate(self.sources):
            if movment is None:
                ...
                color_sub_plain = my_cmap(norm(movment[i]))
            rect = patches.Rectangle((sr.x - sr.length / 2, -sr.y), sr.length, sr.width, edgecolor='k', facecolor=color_sub_plain, zorder=1)
            ax.add_patch(rect)
    ...
    def compute_station_disp(self, s, ss, ds, azimuth, incidence_angle, zero_padd=0):
        ...
        for sr in self.sources:
            ...
            z = np.zeros(1)
            model = np.array(
                [sr.length, sr.width, sr.depth, np.rad2deg(sr.dip), np.rad2deg(sr.strike), 0, 0, ss*self.strike_element, ds*self.dip_element, 0.0],
                dtype='float64')
            ...
            uZ[i] = z[0]
            i += 1
        return -((-np.cos(azimuth) * uE + np.sin(azimuth) * uN) * np.sin(incidence_angle) + uZ * np.cos(incidence_angle))
    ...
    def seismic_moment(self, strike_slip, dip_slip, convert_to_meter=1e3, mu=30e9):
        seismic_moment = 0
        for i, sr in enumerate(self.sources):
            seismic_moment += sr.length * convert_to_meter * sr.width * convert_to_meter * mu * np.sqrt(
                strike_slip[i] ** 2 + dip_slip[i] ** 2)
