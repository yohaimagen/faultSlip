...
class Image:
    def __init__(self, disp_file=None, lon=None, lat=None, x_pixel=None, y_pixel=None, incidence_angle=None, azimuth=None, station=None, station_size=None, plains=None, origin_x=None, origin_y=None, mask_fault=False, **kwargs):
        ...
        if station == 'uniform':
            ...
            e = np.linspace(self.origin_x, self.origin_x + self.im_x_size, self.disp.shape[1], endpoint=False)
            n = np.linspace(self.origin_y, self.origin_y + self.im_y_size, self.disp.shape[0], endpoint=False)
            ...
            for i in range(x_steps.shape[0] - 1):
                for j in range(y_steps.shape[0] - 1):
                    self.station.append(Station(x_steps[i], y_steps[j], x_size=x_steps[i + 1] - x_steps[i],
                                                y_size=y_steps[j + 1] - y_steps[j]))
        else:
            # st = pd.read_csv(station, sep=' ', header=None)
            # st = st.as_matrix()
            st = np.load(station)
            self.station = [Station(x, y, x_size, y_size, flag=True) for x, y, x_size, y_size in
                       zip(st[:, 0], st[:, 1], st[:, 2], st[:, 3])]
        ...
        if mask_fault:
            for p in plains:
                ...
                X = np.linspace(x_start, x_end, int(np.abs(x_start / self.x_pixel-x_end / self.x_pixel))*3)
                Y = m*X + n
                X = X / self.x_pixel
                Y = Y / self.y_pixel
                X = X.astype(np.int)
                ...
                mask = np.logical_and(Y >= 0, np.logical_and(Y < self.disp.shape[0], np.logical_and(X >= 0, X < self.disp.shape[1])))
                X = X[mask]
                Y = Y[mask]
    ...
    def build_kernal(self, ss, ds, plains, poisson_ratio=0.25):
        def build(plain, ss, ds):
            ...
            for i, sr in enumerate(plain.sources):
                ...
                model = np.array(
                    [sr.length, sr.width, sr.depth,  np.rad2deg(sr.dip),
                     np.rad2deg(sr.strike), 0, 0, ss, ds, 0.0],
                    dtype='float64')
                disloc.disloc_1d(uE, uN, uZ, model, east - sr.e, north - sr.n, poisson_ratio, east.shape[0], 1)
                A[i, :, 0] = uE
                A[i, :, 1] = uN
                A[i, :, 2] = uZ
    ...
    def calculate_station_disp(self, S=None, resamp=False):
        ...
        for s in stations:
            ...
            try:
                s.disp = self.disp[int(np.rint(north / self.y_pixel)), int(np.rint(east / self.x_pixel))]
            except :
                print('cords not in data:')
                print(int(np.rint(north / self.y_pixel)), int(np.rint(east / self.x_pixel)))
                exit(1)
            s.weight = 1
    ...
    def drop_crossing_station(self, plains):
        ...
        def line_rect_cross(lx1, ly1, lx2, ly2, rx1, ry1, rx2, ry2, rx3, ry3, rx4, ry4):
            return line_line_cross(lx1, ly1, lx2, ly2, rx1, ry1, rx2, ry2) or\
                   line_line_cross(lx1, ly1, lx2, ly2, rx2, ry2, rx3, ry3) or\
                   line_line_cross(lx1, ly1, lx2, ly2, rx3, ry3, rx4, ry4) or\
                   line_line_cross(lx1, ly1, lx2, ly2, rx4, ry4, rx1, ry1)
        ...
        for i, st in enumerate(self.station):
            ...
            for x_fault, y_fault in zip(x_faults, y_faults):
                st_cross = st_cross or line_rect_cross(x_fault[0], y_fault[0], x_fault[1], y_fault[1],
    ...
    def full_model(self, strike_slip, dip_slip, poisson_ratio, zero_pad, plains):
        E, N = np.meshgrid(np.linspace(self.origin_x, self.origin_x + self.im_x_size, self.disp.shape[1], endpoint=False),
                           np.linspace(self.origin_y, self.origin_y + self.im_y_size, self.disp.shape[0], endpoint=False))
        ...
        for p in plains:
            for s in p.sources:
                ...
                model = np.array(
                    [s.length, s.width, s.depth, np.rad2deg(s.dip),
                     np.rad2deg(s.strike), 0, 0, strike_slip[i] * p.strike_element, dip_slip[i] * p.dip_element, 0.0],
                    dtype='float64')
                disloc.disloc_2d(uE_t, uN_t, uZ_t, model, E - s.e, N - s.n, poisson_ratio, E.shape[0] * E.shape[1],
                                 1)
                uE += uE_t
                uN += uN_t
                uZ += uZ_t
                i += 1
            i += zero_pad
        los_dis = -(np.sin(self.incidence_angle) * (-np.cos(self.azimuth) * uE + np.sin(self.azimuth) * uN) + np.cos(self.incidence_angle) * uZ)
        self.model = los_dis
        return los_dis, uE, uN, uZ

    def full_model_m(self, strike_slip, dip_slip, poisson_ratio, zero_pad, plains):
        E, N = np.meshgrid(np.linspace(0, self.im_x_size, self.disp.shape[1], endpoint=False),
        ...
        for p in plains:
            for s in p.sources:
                ...
                model = np.array(
                    [s.length, s.width, s.depth, np.rad2deg(s.dip),
                     np.rad2deg(s.strike), 0, 0, strike_slip[i], dip_slip[i], 0.0],
                    dtype='float64')
                disloc.disloc_2d_m(uE_t, uN_t, uZ_t, model, E - s.e, N - s.n, poisson_ratio, E.shape[0] * E.shape[1],
                                   1)
                uE += uE_t
                uN += uN_t
                uZ += uZ_t
    ...
    def plot_sol(self, ax1, ax2, ax3, strike_slip, dip_slip, plains, vmin, vmax, cmap='jet', poisson_ratio=0.25, zero_pad=0):
        def m2dd(m, lat=0):
        ...
        tic_y = np.linspace(self.origin_x, self.origin_x + self.disp.shape[0], 3, dtype=np.int)
        tic_x = np.linspace(self.origin_y, self.origin_y + self.disp.shape[1], 4, dtype=np.int)[1: -1]
        ...
        for ax, img in zip((ax1, ax2, ax3), (self.disp, model[0], self.disp - model[0])):
            im = ax.imshow(img, cmap=cmap, aspect=self.y_pixel / self.x_pixel, origin='lower', vmax=vmax, vmin=vmin,)
...
'''
    ...
    def plot_model(self, ax, strike_slip, dip_slip, vmin, vmax, plains, cmap='jet', poisson_ratio=0.25, zero_pad=0, img=None):
        def m2dd(m, lat=0):
            'meters to decimal degrees.'
            return m / 111319.9 / cos(np.deg2rad(lat))
        def tick_to_strings(ticks):
            ...
            return str_array
        if img is None:
            img = self.full_model(strike_slip, dip_slip, poisson_ratio, zero_pad, plains)
        X, Y = self.get_fault(plains)
        tic_y = np.linspace(self.disp.shape[0] * 0.1, self.disp.shape[0] * 0.9, 3, dtype=np.int)
        tic_x = np.linspace(self.disp.shape[1] * 0.2, self.disp.shape[1] * 0.8, 2, dtype=np.int)
        tic_y_label = tick_to_strings(m2dd(tic_y / self.y_pixel, self.lon) + self.lat)
        tic_x_label = tick_to_strings(m2dd(tic_x / self.x_pixel) + self.lon)
        im = ax.imshow(img, cmap=cmap, aspect=self.y_pixel / self.x_pixel, origin='lower', vmax=vmax, vmin=vmin,)
        ax.set_ylim(0, self.disp.shape[0])
    ...
    def plot_stations(self, ax, plains, dots=True, cmap='jet', vmax=0.2, vmin=-0.2):
        ax.imshow(self.disp, cmap=cmap, aspect=self.y_pixel/self.x_pixel, origin='lower', vmax=vmax, vmin=vmin, extent=(0, self.im_x_size, 0, self.im_y_size))
        if dots:
            ...
            for s in self.station:
                ax.add_patch(patches.Rectangle((s.east - self.origin_x - s.x_size / 2.0, s.north - self.origin_y - s.y_size / 2.0), s.x_size ,
                                                       s.y_size, fill=False, ec='k'))
        X, Y = self.get_fault(plains)
        for x,y in zip(X, Y):
    ...
    def plot_stations_val(self, ax, plains, cmap='jet', vmax=0.2, vmin=-0.2, disp=None):
        ...
        for s, d, i in zip(self.station, disp, range(len(self.station))):
            ...
            ax.add_patch(
                patches.Rectangle((s.east - self.origin_x - s.x_size / 2.0, s.north - self.origin_y - s.y_size / 2.0),
                                  s.x_size,
                                  s.y_size, facecolor=s_color, ec=s_color))
    ...
    def compute_station_disp1(self, s, ss, ds, plains, sources_mat, poisson_ratio=0.25):
        ...
        model[:, -2] = np.concatenate([np.ones(len(p.sources)) * p.dip_element for p in plains]) * ds
        model[:, -3] = np.concatenate([np.ones(len(p.sources)) * p.strike_element for p in plains]) * ss
        model = model.flatten()
        ...
        disp = -((-np.cos(self.azimuth) * uE + np.sin(self.azimuth) * uN) * np.sin(self.incidence_angle) + uZ * np.cos(
    ...
    def compute_source_disp(self, sr, ss, ds, zero_padd=0, poisson_ratio=0.25):
        ...
        model = np.array(
            [sr.length, sr.width, sr.depth, np.rad2deg(sr.dip), np.rad2deg(sr.strike), 0, 0, ss, ds, 0.0],
            dtype='float64', order='C')
        disloc.disloc_1d(uE, uN, uZ, model, east - sr.e, north - sr.n, poisson_ratio, station_num, 1)
        return -((-np.cos(self.azimuth) * uE + np.sin(self.azimuth) * uN) * np.sin(self.incidence_angle) + uZ * np.cos(
            self.incidence_angle))
    ...
    def compute_station_disp(self, s, ss, ds, plains):
        ...
        for plain in plains:
            for sr in plain.sources:
                ...
                z = np.zeros(1)
                model = np.array(
                    [sr.length, sr.width, sr.depth, np.rad2deg(sr.dip), np.rad2deg(sr.strike), 0, 0, ss * p.strike_element, ds * p.dip_element, 0.0],
                    dtype='float64')
                disloc.disloc_1d(e, n, z, model, np.array([s.east - sr.e]), np.array([s.north - sr.n]), 0.25, 1, 1)
                uE[i] = e[0]
                uN[i] = n[0]
                uZ[i] = z[0]
                i += 1
        return -((-np.cos(self.azimuth) * uE + np.sin(self.azimuth) * uN) * np.sin(self.incidence_angle) + uZ * np.cos(self.incidence_angle))

    def resample_model(self, source_plain, source_ind, mat_ind, strike_element, dip_element, weights, poisson_ratio, plains):
        ...
        if strike_element != 0:
            ...
            for new_sr, k in zip(SR, [mat_ind, mat_ind + 1, mat_ind + 2, mat_ind + 3]):
                B_strike = np.insert(B_strike, k,
                                     self.compute_source_disp(new_sr,
                                                              plains[source_plain].strike_element * strike_element,
                                                              0, poisson_ratio=poisson_ratio), axis=1)
        ...
        if dip_element != 0:
            ...
            for new_sr, k in zip(SR, [mat_ind, mat_ind + 1, mat_ind + 2, mat_ind + 3]):
                B_dip = np.insert(B_dip, k, self.compute_source_disp(new_sr, 0, plains[source_plain].dip_element*dip_element, poisson_ratio=poisson_ratio),
                                  axis=1)
        else:
            B_dip = self.get_G_ds()
    ...
    def resample(self, station_ind, strike_element, dip_element, plains, weights, poisson_ratio, surces_mat):
        s = self.station[station_ind]
        S = s.make_new_stations()
        if strike_element != 0:
            B_strike = np.delete(self.get_G_ss(), station_ind, 0)
            for s, k in zip(S, [station_ind, station_ind + 1, station_ind + 2, station_ind + 3]):
                B_strike = np.insert(B_strike, k,
                                     self.compute_station_disp1(s, strike_element, 0, plains, surces_mat, poisson_ratio=poisson_ratio), axis=0)
        else:
            B_strike = np.zeros((self.get_G_ss().shape[0] + 3, 0))
        if dip_element != 0:
            B_dip = np.delete(self.get_G_ds(), station_ind, 0)
            for s, k in zip(S, [station_ind, station_ind + 1, station_ind + 2, station_ind + 3]):
                B_dip = np.insert(B_dip, k, self.compute_station_disp1(s, 0, dip_element, plains, surces_mat, poisson_ratio=poisson_ratio), axis=0)
        else:
            B_dip = np.zeros((self.get_G_ds().shape[0] + 3, 0))
        G = np.concatenate((B_strike, B_dip), axis=1)
        if weights:
            w_normalize, t_station_mat = self.calculate_station_disp_weight(S, self.w_normalizer - s.weight)
            t_station_mat = np.insert(np.delete(self.stations_mat, station_ind, 0), station_ind, t_station_mat, 0)
    ...
    def insert_row(self, ind, station, strike_element, dip_element, plains):
        if strike_element != 0:
            ...
            for i, s in enumerate(station):
                self.G_ss = np.insert(self.G_ss.T, ind + i,
                                               self.compute_station_disp(s, strike_element, 0, plains), axis=1).T
        else:
            self.strike_kernal = np.zeros((self.G_ss.shape[0] + 3, 0))
        if dip_element != 0:
            self.G_ds = np.delete(self.G_ds, ind, axis=0)
            for i, s in enumerate(station):
                self.G_ds = np.insert(self.G_ds.T, ind + i,
                                               self.compute_station_disp(s, 0, dip_element, plains), axis=1).T
        else:
            self.G_ds = np.zeros((self.G_ds.shape[0] + 3, 0))

    def add_new_stations(self, indices, strike_element, dip_element, plains, weights=False):
        ...
        for i in indices:
            ...
            S = s.make_new_stations()
            if weights:
                self.w_normalizer, t_stations_mat = self.calculate_station_disp_weight(S, self.w_normalizer - s.weight)
            else:
                self.w_normalizer, t_stations_mat = self.calculate_station_disp(S)
            self.stations_mat = np.insert(np.delete(self.stations_mat, shifted, axis=0), shifted, t_stations_mat, axis=0)
    ...
    def add_new_source(self, plain_inds, sources_inds, mat_inds, strike_element, dip_element, plains, weights=False, poisson_ratio=0.25):
        new_sources = []
        shift_mat = 0
            ...
            for i in range(t_mat_inds.shape[0]):
                ...
                for s in SR:
                    mat_in.append(np.array(
                        [s.length, s.width, s.depth, np.rad2deg(s.dip), np.rad2deg(s.strike), 0, 0, 0, 0, 0, s.e, s.n, s.x, s.y],
                        dtype='float64'))
                mat_in = np.vstack(mat_in)
                shifted_mat = t_mat_inds[i] + shift_mat
                shift_mat += 3
                self.sources_mat = np.insert(np.delete(self.sources_mat, shifted_mat, axis=0), shifted_mat, mat_in, axis=0)
                plain.insert_column(self.station, shifted, SR, strike_element,
                                                         dip_element, self.azimuth, self.incidence_angle, poisson_ratio, zero_pad=0)
                self.sources_num += 3
    ...
    def stations_to_csv(self, path, index):
        stations_mat = np.array([[s.east, s.north, s.x_size, s.y_size] for s in self.station] )
    ...
    def restore_clean_displacment(self):
        assert self.clean is not None, 'add_uncorelated_noise() or add_corelated_noise(), shold be run befor restoring clean displacment'
    ...
    def quadtree(self, thershold, min_size, max_grad):
        def quad(s):
            ...
            if not_nans > 2 and lux > 1 and luy > 1 and lux/luy < 3 and lux/luy > 1/3:
                std = np.nanstd(data)
    ...
    def insert_column(self, mat_ind, SR, strike_element, dip_element, poisson_ratio=0.25):
        east = np.array([st.east for st in self.station], dtype='float64')
        north = np.array([st.north for st in self.station], dtype='float64')
        def calc_disp(ss, ds):
            ...
            return -((x + y) * np.sin(self.incidence_angle) + z)
        if strike_element != 0:
            self.G_ss = np.insert(np.delete(self.G_ss, mat_ind, axis=1), mat_ind, calc_disp(strike_element, 0).T, axis=1)
        if dip_element != 0:
            self.G_ds = np.insert(np.delete(self.G_ds, mat_ind, axis=1), mat_ind, calc_disp(0, dip_element).T, axis=1)
