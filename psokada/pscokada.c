...
void *strain(void *in_arg)
{
    ...
    for(int i = 0; i < args->pop_mum; i++)
    {
        c_okada_strain(args->x1, args->y1, args->z1, args->strike, args->dip, args->L, args->W, args->slip_strike,
         args->slip_dip, args->open, args->x2[i], args->y2[i], args->z2[i], args->s + (i * 9), args->lame_lambda, args->mu);
    }
    return NULL;
}
		...
		double lame_lambda, double mu)
{
	...
	strain_okada(x1, y1, z1, strike1, dip1, L, W, slip_strike, slip_dip, open, x2, y2, z2, &s[0], &s[4], &s[8], &s[1], &s[2], &s[5], lame_lambda, mu);
	s[3] = s[1];
	s[6] = s[2];
	s[7] = s[5];
 ...
 */
	...
	/* transform from Okada's to Aki's system */
	*sxx = UXX * cos_stk * cos_stk + UYY * sin_stk * sin_stk + 0.5 * (UXY + UYX) * sin_2stk;
	*syy = UXX * sin_stk * sin_stk + UYY * cos_stk * cos_stk - 0.5 * (UXY + UYX) * sin_2stk;
	*szz = UZZ;
	*sxy = 0.5 * (UXX - UYY) * sin_2stk - 0.5 * (UXY + UYX) * cos(2.0 * stk);
	dum1 = -0.5 * (UZX + UXZ);
	dum2 = 0.5 * (UYZ + UZY);
	*syz = dum1 * sin_stk + dum2 * cos_stk;
	*szx = dum1 * cos_stk - dum2 * sin_stk;
}
