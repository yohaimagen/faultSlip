...
class Genrate_catalog:
    ...
    def as_contribution(self, inv: Inversion):
        ...
        expected_as = []
        NT = (
            (self.T - self.catalog[:, 3] + self.c) ** (1 - self.p)
            - self.c ** (1 - self.p)
        ) / (1 - self.p)
    ...
    def get_Gf_inds(self, s):
        ...
        if s.length == s.width:
            ...
            if l - w > w / 2.0:
                raise Exception(
                    "ratio between dislocation length and width more than 1.5"
                )
            else:
    ...
    def calc_kernal(self, inv: Inversion, shear_m=30e9):
        ...
        omori = ((self.T + self.c) ** (1 - self.p) - self.c ** (1 - self.p)) / (
            1 - self.p
        )
        inds = []
        for p in inv.plains:
        ...
        for p in inv.plains:
            for s in p.sources:
                gr = (
                    ((shear_m * s.length * s.width * 1e6) ** (2.0 / 3))
                    / (10 ** (9.05 / 1.5 + self.m_min))
                ) * self.k
    ...
    def as_in_disloc(self, inv: Inversion, min_dis=3.0):
        ...
        for p in inv.plains:
            for s in p.sources:
                disloc_corners = np.concatenate(
                    (disloc_corners, s.get_corners().reshape(1, 4, 3)), axis=0
                )
        dists = self.distance(
            disloc_corners[:, 0],
            disloc_corners[:, 1],
    ...
    def distance(self, t1, t2, t3, t4, p):
        ...
        pp = np.stack(
            (np.einsum("ij,ikj->ki", v, pp), np.einsum("ij,ikj->ki", u, pp))
        ).T
        ...
        D_plain[mask_corners] = np.min(
            np.linalg.norm(
                np.repeat(T.reshape(-1, 1, 4, 2), pp.shape[1], axis=1)[mask_corners]
                - pp[mask_corners].reshape(-1, 1, 2),
                axis=2,
            ),
            axis=1,
        )
        ...
        for plain in inv.plains:
            for isr, sr in enumerate(plain.sources):
                if sr.strike_slip < 1e-7 and sr.dip_slip < 1e-7:
                    continue
                strain += sr.strain_thread(
                    X[:, 0],
                    X[:, 1],
                    X[:, 2],
                    inv.strike_element * plain.strike_element,
                    inv.dip_element * plain.dip_element,
                    lambda_l,
                    shear_m,
                )
    ...
    def calc_strain_ps(self, X, mw, x, y, z):
        ...
        for i in range(X.shape[0]):
            stress[i] = py_point_source_strain(
                x,
                y,
                z,
                np.pi / 2.0,
    ...
    def generate_catalog_slip(
        self, inv: Inversion, T, X, kdtree, k=2.84e-3, p=1.07, c=1.78e-5, m_min=2.5
    ):
        ...
        if N < 1:
            ...
            while accepted.shape[0] < N:
                ...
                accepted = np.concatenate(
                    (
                        accepted,
                        np.concatenate((x[mask], t[mask].reshape(-1, 1)), axis=1),
                    ),
                    axis=0,
                )
                ...
                ntrail += 1
            accepted = np.concatenate(
                (accepted, self.sample_magnitudes(mw, m_min, N).reshape(-1, 1)), axis=1
            )
            return accepted

    def generate_catalog_ps(
        self,
        inv: Inversion,
        mw,
        x,
    ...
    def fit_catalog(
        ...
        if load_gf is None:
            ...
            for i in range(cat.shape[0]):
                Gf.append(
                    self.calc_strain_ps(X, cat[i, 5], cat[i, 0], cat[i, 1], cat[i, 2])
                )
            Gf = np.stack(Gf)
        else:
            Gf = np.load(load_gf)
        ...
        args = list(
            itertools.product(
                sranges[0], sranges[1], sranges[2], [Gf], [cat], [kdtree], [T], [2.5]
            )
        )
        ...
        p.close()
        p.join()
        jout = np.array(list(itertools.chain(*results))).reshape(
            sranges[0].shape[0], sranges[1].shape[0], sranges[2].shape[0]
        )
    ...
    def mc_fit_catalog(
        ...
        for k in range(samples):
            args.append(
                (sranges[0][k], sranges[1][k], sranges[2][k], Gf, cat, kdtree, T, 2.5)
            )
...
def logliklihod(k, c, p, Gf, cat, kdtree, T, m_min):
    ...
    for i in range(cat.shape[0]):
        ...
        for j in range(1, i + 1):
            ll_i += (
                k
                * np.power(10, cat[j, 5] - m_min)
                * omori(cat[i, 3], cat[j, 3], c, p)
                * Gf[j, ind]
            )
        ll += np.log(ll_i)
    return ll - k * LL
