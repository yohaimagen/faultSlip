...
class Source:
    def __init__(
        ...
        self.ccw_to_x_stk = (
            np.pi / 2 - self.strike
        )  # the angle between the fault and the x axis cunter clock wise
        ...
        if east is not None and north is not None and depth is not None:
            ...
            self.e = (
                a
                + x * np.cos(self.ccw_to_x_stk)
                + np.cos(self.ccw_to_x_dip) * np.cos(dip) * y
            )
            self.n = (
                b
                + x * np.sin(self.ccw_to_x_stk)
                + np.sin(self.ccw_to_x_dip) * np.cos(dip) * y
            )
            self.depth = c + np.sin(dip) * y
            self.e_t = (
                a
                + x * np.cos(self.ccw_to_x_stk)
                + np.cos(self.ccw_to_x_dip) * np.cos(dip) * (y - self.width)
            )
            self.n_t = (
                b
                + x * np.sin(self.ccw_to_x_stk)
                + np.sin(self.ccw_to_x_dip) * np.cos(dip) * (y - self.width)
            )
            self.depth_t = c + np.sin(dip) * (y - self.width)
            self.e_m = (
                a
                + x * np.cos(self.ccw_to_x_stk)
                + np.cos(self.ccw_to_x_dip) * np.cos(dip) * (y - 0.5 * self.width)
            )
            self.n_m = (
                b
                + x * np.sin(self.ccw_to_x_stk)
                + np.sin(self.ccw_to_x_dip) * np.cos(dip) * (y - 0.5 * self.width)
            )
    ...
    def make_new_source(self):
        ...
        e = self.e - (
            self.length / 2 * np.cos(self.ccw_to_x_stk) + np.cos(self.ccw_to_x_dip) * l
        )
        n = self.n - (
            self.length / 2 * np.sin(self.ccw_to_x_stk) + np.sin(self.ccw_to_x_dip) * l
        )
        sr1 = deepcopy(self)
        sr1.length /= 2.0
        sr1.width /= 2.0
        ...
        sr1.e = (
            e
            + sr1.length / 2 * np.cos(self.ccw_to_x_stk)
            + np.cos(self.ccw_to_x_dip) * np.cos(sr1.dip) * sr1.width
        )
        sr1.n = (
            n
            + sr1.length / 2 * np.sin(self.ccw_to_x_stk)
            + np.sin(self.ccw_to_x_dip) * np.cos(sr1.dip) * sr1.width
        )
        ...
        sr2.e = (
            e
            + sr2.length * 1.5 * np.cos(self.ccw_to_x_stk)
            + np.cos(self.ccw_to_x_dip) * np.cos(sr2.dip) * sr2.width
        )
        sr2.n = (
            n
            + sr2.length * 1.5 * np.sin(self.ccw_to_x_stk)
            + np.sin(self.ccw_to_x_dip) * np.cos(sr2.dip) * sr2.width
        )
        ...
        sr3.e = (
            e
            + sr3.length / 2 * np.cos(self.ccw_to_x_stk)
            + np.cos(self.ccw_to_x_dip) * np.cos(sr3.dip) * sr3.width * 2
        )
        sr3.n = (
            n
            + sr3.length / 2 * np.sin(self.ccw_to_x_stk)
            + np.sin(self.ccw_to_x_dip) * np.cos(sr3.dip) * sr3.width * 2
        )
        ...
        sr4.e = (
            e
            + sr3.length * 1.5 * np.cos(self.ccw_to_x_stk)
            + np.cos(self.ccw_to_x_dip) * np.cos(sr3.dip) * sr3.width * 2
        )
        sr4.n = (
            n
            + sr3.length * 1.5 * np.sin(self.ccw_to_x_stk)
            + np.sin(self.ccw_to_x_dip) * np.cos(sr3.dip) * sr3.width * 2
        )
